...
编译后的 jar 文件位于：`target/Trade-1.0-SNAPSHOT.jar`
...
**解决**：
1. 检查 `paper-plugin.yml` 是否存在
2. 如果存在，删除它（只保留 `plugin.yml`）
3. 重启服务器
...
本项目经历了从初始开发到完善的完整过程，解决了众多技术挑战：

1. **初期开发**：实现基本的交易系统框架
2. **NBT 支持**：添加完整的物品数据保留
3. **GUI 优化**：解决物品放置和交互问题
4. **安全性**：实现严格的 NBT 验证
5. **稳定性**：修复 reload、点击等各种问题
6. **用户体验**：添加自定义标题和提示
